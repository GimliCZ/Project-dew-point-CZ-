...
Adafruit_BMP280 bmp; // inicializace senzoru BMP  / I2C
...
void loop() {
  ...
  if (READBUFFERSEGMENTED[5] == 1) { // čtení teploty má 100 ms delay
    if (!bmp.begin(BMP280_ADRESA)) {
      Serial.println("BMP280 senzor nenalezen!"); //Klasicky aktivní
      !bmp.begin(BMP280_ADRESA);
    }
    readtemp();
  };
  ...
  Internaltemp = thermo.temperatureAsync(rtd,RNOMINAL,RREF); 
    }
...
void Hbridge_control (byte RightPWM, byte LeftPWM, bool &Right_side_current_ALARM, bool &Left_side_current_ALARM) {
  ...
  if (alert_check == 0 && alert_check2 == 0) { // Pokud jsou alerty aktivní, znemožni zápis do pwm, vynuluj je a upozorni uživatele
...
void readtemp() {
  ...
  ENV_PRES = (bmp.readPressure() / 100.00) + korekce;
}
